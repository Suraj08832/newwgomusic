 ...
 */
 ...
 func GetThumb(ctx context.Context, videoid string, playerUsername string) (string, error) {
	...
	raushanText := "Dev :- @DOSTANA_HUB"
	sonaliText := "@crush_hu_tera"
	drawTextRight(rgba, raushanText, thumbWidth-padding, padding, yellow)
	drawTextLeft(rgba, sonaliText, padding, thumbHeight-50, yellow)
...
// drawTextLeft draws text aligned to the left at the specified position.
func drawTextLeft(img *image.RGBA, text string, x, y int, clr color.Color) {
	face := basicfont.Face7x13
...
// drawTextRight draws text aligned to the right at the specified position.
func drawTextRight(img *image.RGBA, text string, x, y int, clr color.Color) {
	face := basicfont.Face7x13
	...
	// Position text so it ends at x (right-aligned)
	point := fixed.Point26_6{
		X: fixed.Int26_6((x - int(width.Ceil())) * 64),
		Y: fixed.Int26_6(y * 64),
	}
